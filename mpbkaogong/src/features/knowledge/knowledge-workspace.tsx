...
import { BookOpenText, ExternalLink, LoaderCircle, MessageSquarePlus, Trash2 } from "lucide-react";
...
function KnowledgeChat({ sessionId, onSessionUpdated, initialPrompt = "" }: { sessionId: string; onSessionUpdated: () => void; initialPrompt?: string }) {
  ...
  return (
    <section className="flex min-h-[36rem] min-w-0 flex-1 flex-col border-y border-foreground/35 bg-card/45 lg:min-h-0">
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-foreground/20 px-4">
...
export function KnowledgeWorkspace({ initialSessions, initialPrompt }: { initialSessions: KnowledgeSessionSummary[]; initialPrompt?: string }) {
  ...
  return (
    <div className="grid min-h-0 gap-3 lg:h-[calc(100dvh-8rem)] lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside className="flex min-h-0 flex-col border-y border-foreground/35 bg-card/35">
        ...
        <div className="flex gap-2 overflow-x-auto p-2 lg:flex-1 lg:flex-col lg:overflow-y-auto">
          {sessions.map((session) => (
            <div key={session.id} className={cn("group flex min-w-52 items-center border", selectedId === session.id ? "border-primary bg-primary/5" : "border-foreground/15")}>
              <button type="button" onClick={() => setSelectedId(session.id)} className="min-w-0 flex-1 px-3 py-2 text-left">
      ...
      {selectedId ? (
        <KnowledgeChat key={`${selectedId}:${initialPrompt ?? ""}`} sessionId={selectedId} initialPrompt={initialPrompt} onSessionUpdated={() => void refreshSessions()} />
      ) : (
        <section className="grid min-h-[30rem] place-items-center border-y border-foreground/35 bg-card/35 p-6 text-center">
          <div><BookOpenText className="mx-auto size-8 text-primary" /><h2 className="mt-3 font-semibold">新建课程知识问答</h2><p className="mt-1 text-sm text-muted-foreground">回答会附上课程分P和视频时间位置。</p><Button className="mt-4" onClick={() => void createSession()}>新建问答</Button></div>
        </section>
      )}
    </div>
  );
