...
- Qdrant：第二阶段用于相似题检索、同类错因案例检索和知识点 RAG；第一版不引入独立向量库。
...
- 第一版不持久化 LangGraph checkpoint；业务结果写入 Agent 相关业务表，调试链路交给 LangSmith trace。后续在长期学习计划、human-in-the-loop 审核或跨天 Agent 任务中再启用持久化 checkpoint。
- 第一版不依赖 RAG；学习教练使用结构化练习数据，讲题助教使用当前题、官方解析和用户答案。第二阶段引入 Qdrant 后，再增加相似题、同类错因和知识点讲解片段检索。
- 第一版只持久化 Agent 业务结果，包括推荐、讲题消息、反馈和用户动作；不把 chain-of-thought、完整 prompt 调试包、LangGraph checkpoint 或 LangSmith trace 写入业务库。
- 学习教练默认结合近 20 场已提交练习和近 7 天趋势进行诊断；诊断窗口和推荐阈值从后台配置读取，并在配置缺失时使用代码默认值兜底。
- 推荐动作不提前创建 `PracticeSession`；只有用户点击推荐后，Route Handler 才调用对应服务创建练习会话，并把新会话写回 `AgentRecommendation.startedSessionId`。
- 讲题助教第一版只在提交后、背题模式和错题复盘中开放；未提交练习不开放讲题助教，也不提供提示模式。
